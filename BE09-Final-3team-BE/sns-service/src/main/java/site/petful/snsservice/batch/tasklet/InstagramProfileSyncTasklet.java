...
@Slf4j
public class InstagramProfileSyncTasklet implements Tasklet {
        ...
        try {
            ...
            for (Long userNo : users) {
                try {
                    ...
                    log.info("사용자 {} 프로필 동기화 완료", userNo);
                } catch (Exception e) {
                    log.error("사용자 {} 프로필 동기화 실패: {}", userNo, e.getMessage());
                }
            }

            log.info("==========Instagram 프로필 동기화 완료==========");
